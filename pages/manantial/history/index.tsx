...
export const getServerSideProps = async () => {
  try {
    const response = await axios.get(
      "http://localhost:3000/api/histories/manantial"
    );
    return {
      props: {
              ...
              {lotesEM.map((ofertEM, index) => {
                return (
                  <tr className="text-center" key={index}>
                    ...
                    <td className="xl:px-6 xl:py-3 px-2 py-1">{ofertEM.cli_totalOferta.toLocaleString("en-US", {
                        style: "currency",
                        currency: "USD",
                      })}</td>
                    <td className="xl:px-6 xl:py-3 px-2 py-1">{ofertEM.cli_state}</td>
                  </tr>
                );
              })}
            </tbody>
