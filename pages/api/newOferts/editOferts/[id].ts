...
const EditOfert = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  dataBase.query(
    "UPDATE oferta SET cli_name = ?, cli_estadoCivil = ?, cli_id = ?, cli_sexo = ?, cli_fecNac = ?, cli_telef = ?, cli_cell = ?, cli_mail = ?, cli_trabajo = ?, cli_cargoT = ?, cli_direccT = ?, cli_telefT = ?, cli_conyuName = ?, cli_conyuID = ?, cli_conyuCell = ?, cli_conyuTrab = ?, cli_conyuDireccT = ?, cli_conyuTelT = ?, cli_descuento = ?, cli_totalOferta = ?, cli_state = ? WHERE id = ?",
    [
      cli_name,
      cli_estadoCivil,
      cli_id,
